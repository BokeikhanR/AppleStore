...
//MARK: - Gesture Functions
    ...
    private func tapToGisture() {
        ...
        let tapSecond = UITapGestureRecognizer(target: self, action: #selector(hundleTap))
        blackUnityView.addGestureRecognizer(tapSecond)
        blackUnityView.isUserInteractionEnabled = true

        let tapThird = UITapGestureRecognizer(target: self, action: #selector(hundleTap))
        caseBownView.addGestureRecognizer(tapThird)
    ...
    @objc func hundleTap(sender: UITapGestureRecognizer, paramButton: UIView) {
        ...
        case inhaceFlatView:
            ...
            productVC.senaText = "3 999.0 Тенге"
        case blackUnityView:
            let productVC = ProductViewController()
            navigationController?.pushViewController(productVC, animated: true)
            productVC.labelText = nameElementLabel.text
            productVC.elementViewImage = blackUnityImageView.image
            productVC.senaText = "5 999.0 Тенге"
        case caseBownView:
            let productVC = ProductViewController()
            navigationController?.pushViewController(productVC, animated: true)
            productVC.labelText = nameElementLabel.text
            productVC.elementViewImage = caseBownImageView.image
            productVC.senaText = "6 999.0 Тенге"
        default:
            break
        }
    }
